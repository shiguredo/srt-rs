...
`src/srt_connection.rs` の `SrtConnection` 構造体に `key_refresh_event_sent: bool` フィールド（初期値 `false`）を追加し、`check_km_refresh` 内の `KeyRefreshNeeded` 発行条件に `!self.key_refresh_event_sent` を追加する。イベント発行時に `self.key_refresh_event_sent = true` を設定し、`decommission_old_key()` の呼び出し後に `self.key_refresh_event_sent = false` でリセットする。

テストは `src/srt_connection.rs` 内の `#[cfg(test)]` モジュールに追加する。`encrypted_packet_count` は `CryptoContext` の private フィールドであり、公開 API 経由では `KM_REFRESH_PERIOD - KM_PRE_ANNOUNCE_PERIOD` (約 3355 万パケット) まで `send()` を呼ぶ必要があるため、`tests/` からは実現できない。`#[cfg(test)]` モジュールであれば private フィールドにアクセスして閾値付近の状態を作れる。検証シナリオは次の 3 つ。

- `send()` を複数回呼んでも `KeyRefreshNeeded` が 1 回しか発行されないこと
- `poll_event()` でイベントを消費後、`provide_new_sek()` を呼ぶ前に `send()` を呼んでも再発行されないこと
- 1 サイクル完了 (`decommission_old_key` 相当の状態遷移) 後に再度 `KeyRefreshNeeded` が発行されること
