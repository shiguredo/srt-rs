...
実害は 60 秒ちょうどのタイムスタンプのパケットにのみ影響し、終了判定の発火が遅れる。ラップ後 ts が [30 秒, 60 秒) の範囲のパケットを 1 つも受信しない場合 (30 秒以上の受信ギャップ・低レート伝送) は `wrapping_period_active` が残留し、次のラップまで終了判定が発火しない可能性があるが、通常運用では発生頻度は極めて低い (ラップ周期 01:11:35 ごと)。#0021 実装後はラップ後パケットの配信時刻補正により実害はさらに限定される。
